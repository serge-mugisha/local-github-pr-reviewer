...
import { claimReview, claimThreadAction } from "./review.js";
...
describe("cross-process review claim", () => {
  ...
  it("returns one durable action id to simultaneous thread-action callers", async () => {
    const dir = await mkdtemp(join(tmpdir(), "reviewer-thread-action-claim-"));
    const dbPath = join(dir, "reviewer.db");
    const setup = new Database(dbPath);
    migrateDatabase(setup);
    setup
    ...
    const barrier = new SharedArrayBuffer(Int32Array.BYTES_PER_ELEMENT);
    const claimSource = claimThreadAction.toString();
    const workerSource = `
      const { parentPort, workerData } = require("node:worker_threads");
      const Database = require("better-sqlite3");
      try {
        const claimThreadAction = eval("(" + workerData.claimSource + ")");
        const database = new Database(workerData.dbPath);
        database.pragma("busy_timeout = 5000");
        const barrier = new Int32Array(workerData.barrier);
        Atomics.add(barrier, 0, 1);
        Atomics.notify(barrier, 0);
        while (Atomics.load(barrier, 0) < 2) Atomics.wait(barrier, 0, 1, 5_000);
        const result = claimThreadAction(database, {
          threadId: 1,
          prId: 1,
          kind: "revalidate",
          input: "",
    ...
    `;
    const runWorker = (workerToken: string) =>
      new Promise<{ actionId: number; created: boolean }>((resolveP, rejectP) => {
        const worker = new Worker(workerSource, {
          eval: true,
          workerData: { dbPath, barrier, claimSource, workerToken },
        });
        worker.once(
          ...
          (message: {
            ok: boolean;
            result?: { actionId: number; created: boolean };
            error?: string;
          }) => {
            if (message.ok && message.result) resolveP(message.result);
            else rejectP(new Error(message.error));
    ...
    try {
      const results = await Promise.all([runWorker("worker-a"), runWorker("worker-b")]);
      expect(new Set(results.map((result) => result.actionId)).size).toBe(1);
      expect(results.filter((result) => result.created)).toHaveLength(1);
      const verify = new Database(dbPath);
      expect(verify.prepare("SELECT COUNT(*) AS count FROM thread_actions").get()).toEqual({
        count: 1,
      });
      verify.close();
    } finally {
